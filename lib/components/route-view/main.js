...
((window, document, undefined) => {
  ...
  class RouteView extends HTMLElement
  {
    ...
    static go(url)
    {
      ...
      //Get the regex
      const matchingRegex = RouteView._findMatchingRegex(url, routeMap.keys());

      const matchingView = routeMap.get(matchingRegex);

      const executeResult = matchingRegex.exec(url);

      //Call switch to view
      RouteView.switchToView(matchingView, executeResult);
    }

    static switchToView(view, executeResult)
    {
      //The update should always be called
      let updatePromise = view.update(executeResult);
